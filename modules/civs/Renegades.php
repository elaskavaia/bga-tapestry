...
class Renegades extends AbsCivilization {
        ...
        if ($advance) {
            ...
            if (!$this->isRenegadeAdvancePossible($tier)) {
                $this->game->notifyWithName(
                    "message",
                    clienttranslate('RENEGADES: not possible to use ability for tier ${tier}, skipping'),
                    [
                        "tier" => $tier,
                    ],
                    $player_id
            ...
            $this->game->queueBenefitNormal(
                ["or" => [BE_RENEGADES_ADV, BE_DECLINE]],
                $player_id,
                reason(CARD_CIVILIZATION, CIV_RENEGADES, $tier)
            );
        }
        return true;
    }
    ...
    function getSingleCubeInfo() {
        ...
        $this->systemAssertTrue("ERR:Renegates:02", $cube);
        return $cube;
    }

    function getCurrentTier($cube = null) {
        if (!$cube) {
            $cube = $this->getSingleCubeInfo();
        }
    ...
    function awardBenefits(int $player_id, int $ben, int $count = 1, string $reason = "") {
        ...
        $this->systemAssertTrue("ERR:Renegates:03", $cube_tier !== null);
        $this->systemAssertTrue("ERR:Renegates:03", $cube_tier >= 0);
        if ($cube_tier >= $tier) {
        ...
        if ($next_benefit) {
            $this->game->benefitCashed($next_benefit);
        } // next was spot benefit, we cancelled it
        // renegade advance
        ...
        $this->game->dbSetStructureLocation(
            ...
            $civ_token_string,
            $income_turn,
            clienttranslate('${player_name} advances on RENEGADES'),
            $player_id
        );
