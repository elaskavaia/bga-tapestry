...
class Traders extends AbsCivilization {
    ...
    function sendTrader(int $player_id, array $extra) {
        ...
        if ($building_type) {
            ...
            $game->userAssertTrue(totranslate('TRADERS can only place income building on opponent\'s territory'), $other_building['card_location_arg'] != $player_id);
        ...
        if ($game->isAdjustments8()) {
            if ($self_control) {
                ...
                $this->effect_allOpponentsGainTerritoryTile($player_id);
                // gain 1 res or 5 vp
    ...
    function effect_allOpponentsGainTerritoryTile(int $player_id) {
        $civ = $this->civ;
        $game = $this->game;
        $reason = reason_civ($civ);
    ...
    function getTraderTargets($player_id) {
        ...
        if (!$game->isAdjustments4or8()) {
            ...
            foreach ($map_data as $coord => $hex) {
                ...
                if ($hex['occupancy'] == 0) {
                    $valid[$coord] = 0;
                } else if ($hex['occupancy'] == 1 && !$game->isHexOwner($player_id, $coord, $map_data)) {
