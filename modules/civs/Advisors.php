...
class Advisors extends AbsCivilization {
    ...
    function awardBenefits(int $player_id, int $ben, int $count = 1, string $reason = '') {
        ...
        switch ($ben) {
            case BE_ADVISORS_OVERTAKE:
                ...
                if ($tapvar == "now") { // WHEN PLAYED
                    $game->queueBenefitInterrupt(['or' => [BE_ADVISORS_OVERTAKE_APPROVE, BE_ADVISORS_OVERTAKE_ADVISE]], $player_id, reason_civ($civ, "$opponent_id:$card_id"));
                } else {
    ...
    function playAdviseResponse(int $player_id, int $card_id) {
        ...
        if ($advised_card_id != $card_id) {
            ...
            $game->notifyWithName("message", clienttranslate('${player_name} declines to play advisors card, ADVISORS get decline bonus'), [], $player_id);
    ...
    function argCivAbilitySingle($player_id, $benefit) {
        ...
        if ($game->isAdjustments8()) {
            ...
            $data['title'] = clienttranslate('Place a player token on a space on an opponent\'s income mat that does not already have one of your player tokens');
            $opponents = $this->game->getOpponentsStartingFromLeft($player_id);
            //$opponents[] = $player_id;
            $data['targets'] = [];
            foreach ($opponents as $opponent_id) {
                if ($opponent_id == PLAYER_SHADOW) continue;
    ...
    public  function isAdvisorsOvertake($card_id, $opponent_id, $ben, $era) {
        ...
        if (!($ben == 64 || $ben == BE_PLAY_TAPESTY_INCOME)) return false;
        $player_id = $game->getCivOwner($civ);
        if ($player_id == $opponent_id || $player_id == null) return false;
